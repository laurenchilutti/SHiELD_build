...
export INPUT_DATA1=${INPUT_DATA}

#Example args for Gaea C4 where YOURGROUPLETTER is replaced with the group letter associated with your account
SBATCHARGS="--account=gfdl_YOURGROUPLETTER --qos=debug --clusters=c4 --output=./stdout/%x.o%j"
sbatch ${SBATCHARGS} --time=00:20:00 --nodes=93 --job-name=C768r15n3 C768r15n3.csh
sbatch ${SBATCHARGS} --time=00:20:00 --nodes=61 --job-name=C768 C768.csh
sbatch ${SBATCHARGS} --time=00:45:00 --nodes=20 --job-name=C48n4 C48n4.csh
...
sbatch ${SBATCHARGS} --time=03:00:00 --nodes=298 --job-name=X_SHiELD C3072_res.csh
sbatch ${SBATCHARGS} --time=00:20:00 --nodes=74 --job-name=C384 C384.csh
#Regional3km input data is unique
export INPUT_DATA="/ncrc/home1/Lauren.Chilutti/Alaska_c3072"
